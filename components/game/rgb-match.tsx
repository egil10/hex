...
export function RgbMatchRound({ onAnswer, phase }: RoundProps) {
  ...
  if (!playing) {
    return (
      <div>
        <Prompt>here&apos;s how your match landed.</Prompt>
        <PrecisionFeedback target={target} guess={rgb} />
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <PrimaryButton onClick={submit} className="mt-7">
        match
      </PrimaryButton>
    </div>
  );
